...
<div class="form-wrapper">
    <div class="form-card">
        <h2 class="form-title">Pembaruan Sirkulasi</h2>
        ...
        <form action="{{ route('peminjamans.update', $peminjaman->id) }}" method="POST">
            ...
            <div class="form-group">
                <label class="form-label">Cendekiawan / Siswa Peminjam</label>
                <select name="siswa_id" class="form-control" required>
                    @foreach($siswas as $siswa)
                        <option value="{{ $siswa->id }}" {{ old('siswa_id', $peminjaman->siswa_id) == $siswa->id ? 'selected' : '' }}>{{ $siswa->nis }} - {{ $siswa->nama }}</option>
                    @endforeach
                </select>
                @error('siswa_id') <span class="form-error"><i class='bx bx-error-circle'></i> {{ $message }}</span> @enderror
            </div>

            <div class="form-group">
                <label class="form-label">Koleksi Pustaka (Buku)</label>
                <select name="buku_id" class="form-control" required>
                    @foreach($bukus as $buku)
                        <option value="{{ $buku->id }}" {{ old('buku_id', $peminjaman->buku_id) == $buku->id ? 'selected' : '' }}>{{ $buku->judul }}</option>
                    @endforeach
                </select>
                @error('buku_id') <span class="form-error"><i class='bx bx-error-circle'></i> {{ $message }}</span> @enderror
            </div>

            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
                <div class="form-group">
                    <label class="form-label">Tanggal Mulai Pinjam</label>
                    <input type="date" name="tanggal_pinjam" value="{{ old('tanggal_pinjam', \Carbon\Carbon::parse($peminjaman->tanggal_pinjam)->format('Y-m-d')) }}" class="form-control" required>
                    @error('tanggal_pinjam') <span class="form-error"><i class='bx bx-error-circle'></i> {{ $message }}</span> @enderror
                </div>

                <div class="form-group">
                    <label class="form-label">Batas Waktu Pengembalian</label>
                    <input type="date" name="batas_kembali" value="{{ old('batas_kembali', \Carbon\Carbon::parse($peminjaman->batas_kembali)->format('Y-m-d')) }}" class="form-control" required>
                    @error('batas_kembali') <span class="form-error"><i class='bx bx-error-circle'></i> {{ $message }}</span> @enderror
                </div>
            </div>

            <div class="form-actions">
                <a href="{{ route('peminjamans.index') }}" class="btn-cancel"><i class='bx bx-x'></i> Batalkan</a>
                <button type="submit" class="btn-save"><i class='bx bx-check-circle'></i> Perbarui Data</button>
            </div>
        </form>
    </div>
</div>
